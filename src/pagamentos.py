...
def guardar_pagamentos():
    log.info("A guardar pagamentos em %s", _FICHEIRO_PAGAMENTOS)
    payload = {
    ...
    try:
        ...
        log.info("Pagamentos guardados com sucesso (%d registos)", len(dados.pagamentos))
    except OSError as e:
        log.critical("Erro ao guardar pagamentos: %s", e)


def carregar_pagamentos():
    if not os.path.exists(_FICHEIRO_PAGAMENTOS):
        log.warning("Ficheiro %s nao encontrado — a iniciar sem dados", _FICHEIRO_PAGAMENTOS)
        return False
    log.info("A carregar pagamentos de %s", _FICHEIRO_PAGAMENTOS)
    try:
        ...
        log.info("Pagamentos carregados com sucesso (%d registos)", len(dados.pagamentos))
        return True
    except (OSError, json.JSONDecodeError, KeyError) as e:
        log.error("Erro ao carregar pagamentos: %s", e)
        return False
...
def _registar_transacao(descricao, valor, tipo="entrada", data=None):
    if data is None:
        data = _data_atual()
    log.info("A registar transacao: tipo='%s' descricao='%s' valor=%.2f", tipo, descricao, valor)
...
def criar_pagamento(id_cliente, valor, plano, dia_pagamento=None):
    log.info("Tentativa de criar pagamento: id_cliente=%d plano='%s' valor=%s", id_cliente, plano, valor)
    carregar_pagamentos()
    if id_cliente not in clientes:
        log.error("Cliente id=%d nao encontrado ao criar pagamento", id_cliente)
        return None
    ...
    guardar_pagamentos()
    log.info("Pagamento id=%d criado: cliente='%s' valor=%.2f", novo_id, nome, float(valor))
    return dados.pagamentos[novo_id]


def listar_pagamentos():
    log.info("A listar todos os pagamentos")
    carregar_pagamentos()
    total = len(dados.pagamentos)
    log.info("Listagem concluida: %d pagamentos", total)
    return list(dados.pagamentos.values())


def buscar_pagamento(id_pagamento):
    log.info("A buscar pagamento id=%d", id_pagamento)
    carregar_pagamentos()
    pag = dados.pagamentos.get(id_pagamento)
    if pag is None:
        log.warning("Pagamento id=%d nao encontrado", id_pagamento)
    return pag


def atualizar_pagamento(id_pagamento, valor=None, plano=None, dia_pagamento=None):
    log.info("Tentativa de atualizar pagamento id=%d", id_pagamento)
    carregar_pagamentos()
    pagamento = dados.pagamentos.get(id_pagamento)
    if not pagamento:
        log.warning("Pagamento id=%d nao encontrado para atualizacao", id_pagamento)
        return None
    ...
    guardar_pagamentos()
    log.info("Pagamento id=%d atualizado com sucesso", id_pagamento)
    return pagamento


def apagar_pagamento(id_pagamento):
    log.info("Tentativa de apagar pagamento id=%d", id_pagamento)
    carregar_pagamentos()
    if id_pagamento in dados.pagamentos:
        del dados.pagamentos[id_pagamento]
        guardar_pagamentos()
        log.info("Pagamento id=%d apagado com sucesso", id_pagamento)
        return True
    log.warning("Pagamento id=%d nao encontrado para apagar", id_pagamento)
    return False


def gerar_pagamentos_fim_do_mes():
    log.info("A gerar pagamentos de fim de mes")
    carregar_pagamentos()
    gerados = 0
    for id_cliente, cliente in clientes.items():
        ...
        if codigo != 200 or not plano_info:
            log.warning("Plano id=%s nao encontrado para cliente id=%d — pagamento ignorado", id_plano, id_cliente)
            continue
        nome_plano   = plano_info[0]
        num_treinos  = plano_info[1]
        preco_treino = plano_info[2]
        total_mensal = round(num_treinos * preco_treino, 2)
        criar_pagamento(id_cliente=id_cliente, valor=total_mensal, plano=nome_plano)
        gerados += 1
    log.info("Fim de mes concluido: %d pagamentos gerados", gerados)
...
def _mostrar_transacoes():
    ...
    for t in transacoes:
        if t["tipo"] == "entrada":
            sinal = _VERDE_B + "+" + _RESET; cor = _VERDE;   total_entrada += t["valor"]
        elif t["tipo"] == "saida":
            sinal = _VERMELHO_B + "-" + _RESET; cor = _VERMELHO; total_saida += t["valor"]
        elif t["tipo"] == "transferencia_entrada":
            sinal = _CIANO_B + "+" + _RESET; cor = _CIANO;   total_entrada += t["valor"]
        elif t["tipo"] == "transferencia_saida":
            sinal = _CIANO_B + "-" + _RESET; cor = _CIANO;   total_saida += t["valor"]
        else:
            sinal = " "; cor = _BRANCO
...
def _adicionar_pagamento():
    carregar_pagamentos()
    if not clientes:
        log.warning("Tentativa de adicionar pagamento sem clientes registados")
    ...
    if codigo == 200 and plano_info:
        ...
        print(_CINZA + "Plano atual: " + _RESET + _MAGENTA + nome_plano_sugerido + _RESET +
              _CINZA + "  Valor mensal: " + _RESET + _VERDE + f"{valor_sugerido:.2f} EUR" + _RESET)
    ...
    if resultado:
        ...
        log.error("Falha ao criar pagamento para cliente id=%d", id_cliente)
...
def _remover_pagamento():
    ...
    if confirmar:
        ...
        log.info("Remocao de pagamento id=%d cancelada pelo utilizador", id_pag)
...
def _adicionar_transferencia():
    ...
    if not descricao:
        log.warning("Descricao vazia ao adicionar transferencia")
    ...
    if tipo_input == "1":
        tipo = "transferencia_entrada"; label = "entrada"
    elif tipo_input == "2":
        tipo = "transferencia_saida";   label = "saida"
    else:
        log.warning("Tipo de transferencia invalido: '%s'", tipo_input)
        print(_VERMELHO_B + "400 Opcao invalida. Usa 1 ou 2." + _RESET)
        _aguardar_enter()
        return
    data_input = input(_AMARELO + "Data (DD/MM/AAAA, Enter = hoje): " + _RESET).strip()
    data       = data_input if data_input else None
    _registar_transacao(descricao=f"Transferencia: {descricao}", valor=valor, tipo=tipo, data=data)
    guardar_pagamentos()
    log.info("Transferencia registada: tipo=%s valor=%.2f descricao='%s'", tipo, valor, descricao)
    ...
    while True:
        ...
        if op == "1":
            ...
            try:
                ...
                log.error("Valor invalido introduzido ao atualizar pagamento")
            ...
            print(_CINZA + "-" * 44 + _RESET)
            log.info("Resumo financeiro: entradas=%.2f saidas=%.2f saldo=%.2f", total_ent, total_sai, saldo_liq)
            _aguardar_enter()
            ...
            log.warning("Opcao invalida no menu pagamentos: '%s'", op)
